    ...
    <div class="container w-4/5 mx-auto">
        ...
        <form action="/blog" class="w-fit mx-auto mb-10">
            ...
            <div class="w-fit flex rounded-md border">
                <input class="w-[600px] p-2 focus:outline-lime-400" type="text" placeholder="Search.." value="{{ request('search') }}" name="search">
                ...
                <article class="mb-8 border border-slate-200 text-center">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-sm mb-2">
                            By <a href="/author/{{ $posts[0]->author->username }}" class="text-orange-400">{{ $posts[0]->author->name }}</a> on <a href="/blog?category={{ $posts[0]->category->slug }}" class="underline text-blue-400">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}
                        </p>
